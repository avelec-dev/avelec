 ...
 */
class pdf_facto extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (method_exists($pdf,'AliasNbPages')) if (method_exists($pdf,'AliasNbPages')) $pdf->AliasNbPages();
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->rect($this->marge_gauche - 5, $posy + 0, 80, 40);
		// Logo
		
		$logo=$conf->mycompany->dir_output.'/logos/facto.jpg';
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				$pdf->Image($logo, $this->marge_gauche, $posy + 0, 0, 22);	// width=0 (auto), max height=24
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
		...
		$pdf->SetFont('','B', $default_font_size - 1);
				$pdf->SetXY($this->marge_gauche, $posy + 20);
				$pdf->MultiCell(80, 5, 'Code vendeur :       016589', 0, 'L');
				$pdf->MultiCell(200, 5, 'Num facture'." : " . $outputlangs->convToOutputCharset($object->ref), '', 'L');
				$pdf->MultiCell(100, 5, 'Date facture'." : " . dol_print_date($object->date,"day",false,$outputlangs), '', 'L');
				$pdf->MultiCell(80, 5, 'Montant'." : " . price($object->total_ttc)." euros" , 0, 'L');
				

			
		}

		$logo=$conf->mycompany->dir_output.'/logos/rib_facto.jpg';
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				$pdf->Image($logo, $this->marge_gauche + 120, $posy + 6, 0, 30);	// width=0 (auto), max height=24
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
				...
				$pdf->Image($logo, $this->marge_gauche, 4, 0, 36);	// width=0 (auto)
			}
			else
			{
				$pdf->SetTextColor(200,0,0);
		...
		//$pdf->SetFont('','', $default_font_size - 2);
		 ...
		 $pdf->MultiCell(200, 4, $title." : " . $outputlangs->convToOutputCharset($object->ref), '', 'R');
		$posy+=2;
		$pdf->SetFont('','', $default_font_size - 2);
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DateEndExecution")." : " . dol_print_date($object->date,"day",false,$outputlangs), '', 'R');
