 ...
 */
class pdf_propale_DEVIS extends ModelePDFPropales
{
	...
	function pdf_propale_DEVIS($db)
	{
		...
		$this->db = $db;
		$this->name = "DEVIS";
		$this->description = $langs->trans('DocModelDEVISDescription');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		*/
		...
		//$cury=max($cury,230);
		$cury = 235;
			$deltax=$this->marge_gauche+115;
			$pdf->SetDrawColor(0,0,0);	
			$pdf->SetLineWidth(0.2835);
			$pdf->Rect($deltax - 3,$cury+3,82,40); 
			$pdf->SetFont('','B', $default_font_size - 2);
			$pdf->SetXY($deltax, $cury+5);
			$pdf->MultiCell(85, 5, "Bon pour accord:", 0, 'L');
			$pdf->SetFont('','B', $default_font_size - 2);
			$pdf->SetXY($deltax, $cury+10);
			$pdf->SetFont('','B', $default_font_size - 3);
			$pdf->MultiCell(95, 3, "La signature de cette offre par le client aura valeur d'acceptation.",0,'L',0);
			$pdf->SetXY($deltax, $cury+15);
			$pdf->SetFont('','B', $default_font_size - 2);
			$pdf->MultiCell(85, 3, "Le ___ / ___ / ______ A ________________", 0, 'L', 0);
	...
	function _pagehead(&$pdf, $object, $showaddress=1, $outputlangs)
	{
		...
		$pdf->MultiCell(100, 4, $title.": " . $outputlangs->convToOutputCharset($object->ref), '' , 'R');
